...
def voltage_to_curtis_voltage(voltage):
    return int(voltage * 100)  # in 0.01V
...
def create_curtis_f_node(id):
    ...
    node.sdo["Motor Torque"].raw = motor_torque
...
def simulate_curtis_f_data(node, update_interval=0.125):
    while True:
        ...
        node.sdo["Motor Torque"].raw = random.randint(*motor_torque_range)
        node.sdo["Controller Temperature"].raw = temperature_to_curtis_temperature(
